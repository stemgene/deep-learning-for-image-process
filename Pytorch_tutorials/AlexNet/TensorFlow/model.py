...
# Keras function的方式搭建模型
def AlexNet_v1(im_height=224, im_width=224, class_num=1000):
    # TF中的tensor通道排序是NHWC
    input_image = layers.Input(shape=(im_height, im_width, 3), dtype='float32')    # output = (none, 224, 224, 3)
    # 通过TF的valid或SAME padding都无法得到55的size，所以采用ZeroPadding2D的方法先做一个预处理
    ...
    x = layers.Conv2D(192, kernel_size=5, padding='same', activation='relu')(x)    # output = (none, 13, 13, 192)
    x = layers.Conv2D(192, kernel_size=5, padding='same', activation='relu')(x)    # output = (none, 13, 13, 192)
    x = layers.Conv2D(128, kernel_size=5, padding='same', activation='relu')(x)    # output = (none, 13, 13, 128)
