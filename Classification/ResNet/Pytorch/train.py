...
def main():
    ...
    data_transform = {
        ...
        "val": transforms.Compose([transforms.Resize(256), # 保持高宽比不变，将最小边缩放到256
    ...
    # 需要放到net.load_state_dict等模型读取之后，再送到GPU中
    net.to(device) 
    """
    还有一种载入模型并修改全连接层参数的方式：载入预训练好的参数，得到一个dict，删掉全连接层的参数部分，在主程序中可以修改net = resnet34(classes=5)
    """
