...
class Block(layers.Layer):
    ...
    def __init__(self, dim, frop_rate=0., layer_scale_init_value=1e-6, name: str = None):
        ...
        self.pwconv1 = layers.Dense(4 * dim,  # 用全连接层实现1*1卷积，输出是4*dim
    ...
    # layer scale: 对每个channel的数据进行缩放
    def build(self, input_shape):
        if self.layer_scale_init_value > 0:
            self.gamma = self.add_weight(shape=[input_shape[-1]], # 特征层的channel维度，和gamma的个数一致
                                         initializer = initializers.Constant(self.layer_scale_init_value),
                                         trainable=True,  # 可训练参数
                                         dtype=tf.float32,
                                         name="gamma")
        else:
            self.gamma = None
...
class ConvNeXt(Model):
    ...
    def __init__(self, num_classes: int, depths: list, dims: list, drop_path_rate: float = 0.,
                 layer_scale_init_value: float = 1e-6) -> None:
        super().__init__()
